...
from GenerateData_Astroid import Data


######### exact solution ###############
p=1   # p in the article 

def u(x,label):

    x=x.t()  
    if label=='inner':
        u=(1/(1+p*(x[0]**2+x[1]**2))).view(-1,1)
    elif label=='out':
        u=(2/(1+p*(x[0]**2+x[1]**2))).view(-1,1)
    else:
...
def f_grad(x,label):
    x=x.t()
    v=1+p*(x[0]**2+x[1]**2)
    uxx=(-2*p*v+2*(2*p*x[0])**2)/v**3
    uyy=(-2*p*v+2*(2*p*x[1])**2)/v**3
...
def inter_dirich(x):
    ...
    return (1/(1+p*(x[0]**2+x[1]**2))).view(-1,1)
...
def main(args):
    ...
    test_num=test_inner.size()[0]+test_out.size()[0]
    ...
    for epoch in range(args.nepochs):      
        ...
        if (epoch+1)%args.print_num==0:
            ...
            with torch.no_grad():    
                ...
                print('Epoch,  Training MSE (lr): ',epoch+1,Mse_train,optimizer.param_groups[0]['lr'])
    ...
    print('Rel. L_2:',(L2_inner_loss.item()*test_inner.size()[0]+L2_out_loss.item()*test_out.size()[0])/test_num)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--inner_unit', type=int, default=100)
    parser.add_argument('--out_unit', type=int, default=100)   
