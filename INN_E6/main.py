...
def u(x,label,b,device):
    ...
    if label=='inner':
        ...
        u=b[1]*(torch.cos(x[0]+x[1]+x[2])).view(-1,1).to(device)*(torch.tensor([1.,1.,1.])).to(device) 
    elif label=='inner_numan':         
        u=b[0]*(torch.cat(((x[1]*x[2]).view(-1,1),(x[0]*x[2]).view(-1,1),(x[1]*x[0]).view(-1,1)),1)*(torch.exp(x[0]*x[1]*x[2])).view(-1,1)).to(device)
    else:     
...
def f_grad(x,label,a,b,device):
    xt=x.t() 
    if label=='inner':    
        f=(b[0]-a[0]*((xt[0]*xt[1])**2+(xt[0]*xt[2])**2+(xt[2]*xt[1])**2)).view(-1,1)*u(x,label,a,device) 
    elif label=='out':
...
def inner_or_not(input,pqrfile):
    centers,rs,_=read_pqr(pqrfile,device='cpu',ratio=10)
...
def main(args):
    ...
    ### train data
    data=Data(args.pqrfile,args.box,device,ratio=10)
    ...
    Ninput_boundary=data.SampleFromBoundary(args.Ntrain_boundary)
    index=random.sample(range(0,Ninput_boundary.size()[0]),args.train_boundary)
    input_boundary=Ninput_boundary[index,:]
    input_boundary_label=u(input_boundary,'out',args.a,device)

    g_D = u(input_in_b,'out',a,device)-u(input_in_b,'inner',a,device)
    g_N = phi_grad(input_in_b,f_direction,a,device)
    z=torch.ones(g_D.size()).to(device)
 
    print('input_in_b ',input_in_b.size())
    ...
    for epoch in range(args.nepochs):
        ...
        loss_in=torch.mean((-a[0]*(U_1xx+U_1yy+U_1zz)+b[0]*U1-f_grad(input_in,'inner',a,b,device))**2)
        ...
        loss_out=torch.mean((-a[1]*(U_2xx+U_2yy+U_2zz)+b[1]*U2-f_grad(input_out,'out',a,b,device))**2)       
        ...
        if (epoch+1)%args.print_num==0:
            ...
            with torch.no_grad():    
                ...
                print('*****************************************************',test_out.size()[0]+test_inner.size()[0],68921)  
...
if __name__ == '__main__':
    ...
    parser.add_argument('--pqrfile',type=str, default='transfer_adp.pqr')
    ...
    parser.add_argument('--box', type=list, default=[-2,2,-2,2,-2,2])
