...
def test_data_net(device,args):  
    ...
    label_inner=u(test_inner,'inner').clone().detach()
    test_out=out.float().to(device).clone().detach()
    label_out=u(test_out,'out').clone().detach()
...
def main(args):
    ...
    for epoch in range(args.nepochs):
        ...
        G_NN=((alpha_coefficient(input_gamma,'out',device)*dU2_N
            -alpha_coefficient(input_gamma,'inner',device)*dU1_N)*f_direction).sum(dim=1).view(-1,1)   
...
if __name__ == '__main__':
    ...
    parser.add_argument('--train_out', type=int, default=1500)   
    parser.add_argument('--train_boundary', type=int, default=400) 
    parser.add_argument('--inner_unit', type=int, default=40)
    parser.add_argument('--out_unit', type=int, default=40)
