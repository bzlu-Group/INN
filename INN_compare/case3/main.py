...
def u(x,label):
    x=x.T  
    if label=='inner':
        u=7+(torch.sin(4*pi*x[0])*torch.sin(4*pi*x[1])).view(-1,1) 
    elif label=='out':
        u=5*(torch.exp(-x[0]**2-x[1]**2)).view(-1,1) 
    else:
...
def f_grad(x,label,device):
    xt=x.t() 
    if label=='inner':  
        f = 4*(4*pi)**2*(torch.sin(4*pi*xt[0])*torch.sin(4*pi*xt[1])).view(-1,1)
    elif label=='out':
        f = -3*(4*xt[0]**2+4*xt[1]**2-4).view(-1,1)*u(x,label)     
    else:
        raise ValueError("invalid label for u(x)")
...
def phi_grad(x,f_direction,device): 
    ...
    du_out =torch.autograd.grad(u(x,'out'),x, grad_outputs=z, create_graph=True)[0]*3
    du_in =torch.autograd.grad(u(x,'inner'),x, grad_outputs=z, create_graph=True)[0]*2
    dU=du_out-du_in
...
def main(args):
    ...
    for epoch in range(args.nepochs):
        ...
        loss_in=torch.mean((-2*(U_1xx+U_1yy)-f_grad(input_in,'inner',device))**2) ############
        ...
        G_NN=((dU2_N*3-dU1_N*2)*f_direction).sum(dim=1).view(-1,1)   
        ...
        loss_out=torch.mean((-3*(U_2xx+U_2yy)-f_grad(input_out,'out',device))**2)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--train_gamma', type=int, default=400)   #400/3200/400
    parser.add_argument('--train_out', type=int, default=4800)    #400/4800/800
    ...
    parser.add_argument('--r0', type=float, default=2/3)
